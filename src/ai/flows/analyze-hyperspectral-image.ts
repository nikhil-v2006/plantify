...
const AnalyzeHyperspectralImageInputSchema = z.object({
  imageData: z.string().describe("The hyperspectral image data, as a data URI."),
  fileName: z.string().describe("The name of the uploaded file.")
});
export type AnalyzeHyperspectralImageInput = z.infer<typeof AnalyzeHyperspectralImageInputSchema>;

const AnalyzeHyperspectralImageOutputSchema = z.object({
  report: z.string().describe('A detailed report on crop health, including nutrient deficiencies, water stress, and disease detection.'),
});
export type AnalyzeHyperspectralImageOutput = z.infer<typeof AnalyzeHyperspectralImageOutputSchema>;

export async function analyzeHyperspectralImage(input: AnalyzeHyperspectralImageInput): Promise<AnalyzeHyperspectralImageOutput> {
...
const analyzeHyperspectralImageFlow = ai.defineFlow(
  ...
  async input => {
    // In a real scenario, this flow would involve complex processing of the hyperspectral data.
    // For this prototype, we'll simulate the analysis with a detailed, structured response
    // that demonstrates the *kind* of information a real analysis would provide.
